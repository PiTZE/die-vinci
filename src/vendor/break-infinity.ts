// break_infinity.js, vendored.
//
// The library Antimatter Dimensions itself uses, written by Patashu, and the
// only third-party code that ships inside this game. Everything that holds a
// number here is one of these: ink reaches 1.8e308 in the first half hour and
// a broken run has been measured at 3.3e271094, which is a long way outside
// what a double can hold and exactly what this was written for.
//
// Vendored rather than depended on, at tag v2.2.0, which is the same version
// the npm package shipped. Three reasons. The published package is compiled
// JavaScript with a .d.ts beside it, and this is the TypeScript it was
// compiled from, so the types are the source rather than a description of it.
// A game whose entire arithmetic is one file has no business having that file
// arrive over a network at build time. And MIT asks for its notice to travel
// with the code, which a node_modules directory nobody commits does not do;
// the licence is beside this file now, and it is the one piece of third-party
// code here that legally requires the credit, where the CC0 sounds and icons
// that require none have had theirs all along.
//
// Kept verbatim below this comment so it can be diffed against upstream and
// re-synced. Do not tidy it to match the house style. There is exactly one
// deviation and it is marked where it happens: the `pad-end` import is now a
// one-line local, so this file has no dependencies of its own at all.
//
//   upstream  https://github.com/Patashu/break_infinity.js
//   licence   MIT, see break-infinity.LICENSE.txt beside this file

// One deviation from upstream, recorded in the header above: this was
// `import padEnd from "pad-end"`, a polyfill for String.prototype.padEnd,
// which the same file already calls natively a few lines further down. padEnd
// is ES2017 and this game targets ES2022, so the polyfill was a network
// dependency for a method that is already there.
const padEnd = (s: string, len: number, fill: string): string => s.padEnd(len, fill);
